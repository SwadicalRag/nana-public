-- GarryFuncs™
-- from https://github.com/willox/hash.js/blob/master/plugins/lua/user_modules/gmod_defines.lua
...
for i = 0, 0xFF do
	local crc = i;
	for j = 0, 7 do
		crc = bit.bxor(bit.rshift(crc,1),(bit.band(crc,1) * Polynomial));
	end
	lookup[i] = crc;
end
local function dw(x) return bit.band(x,0xFFFFFFFF); end

function util.CRC(stuff)

	stuff = tostring(stuff);

	local crc = dw(bit.bnot(0));
	for i = 1, stuff:len() do
		crc = bit.bxor(bit.rshift(crc,8),lookup[bit.bxor(bit.band(crc,0xFF),stuff:byte(i,i))]);
	end

	return dw(bit.bnot(crc));
